...
def main(argv=None):
    ...
    parser.add_argument(
        "--nbins", help="Number of bins in phaseogram profiles", type=int, default=100
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--logeref", help="Reference energy for which the pulsar's weight distribution peaks", type=float, default=4.0
    )
    parser.add_argument(
        "--randomphase", help="Randomize photon phases to test background rate", default=False, action="store_true"
    )
    parser.add_argument(
        "--hout", help="Save results to pickle file", default=False, action="store_true"
    )
    ...
    tl = load_Fermi_TOAs(args.eventfile, weightcolumn=args.weightcol, targetcoord=tc, minweight=args.minWeight, logeref=args.logeref)
