...
class emcee_fitter(Fitter):
    ...
    def phaseogram(
        self, weights=None, bins=16, rotate=0.0, size=5, alpha=0.25, plotfile=None
    ):
        """
...
def main(argv=None):
    ...
    parser.add_argument(
        "--logeref",
        help="Reference energy for which the pulsar's weight distribution peaks.",
        type=float,
        default=4.0)
    parser.add_arguemnt(
        "--randomphase",
        help="Randomize photon phases to test background rate?",
        default=False,
        action="store_true")
    ...
    if burnin >= nsteps:
        ...
        sys.exit(1)
    nbins = 16  # 256 # For likelihood calculation based on gaussians file
    outprof_nbins = 16  # 256 #6 # in the text file, for pygaussfit.py, for instance
    minMJD = args.minMJD
    ...
    if not args.usepickle or (
        ...
        tl = fermi.load_Fermi_TOAs(
            eventfile, weightcolumn=weightcol, targetcoord=target, minweight=minWeight, logeref=args.logeref
        )
